...
export const POST = async (req: Request) => {
  try {
    ...
    if (!isValidObjectId(userId) || !token)
      return NextResponse.json({ error: "خطأ في العملية!" }, { status: 401 });
...
export const GET = async (req: Request) => {
    try {
      ...
      if (!user)
        return NextResponse.json(
          { error: "خطأ في الطلب ,المستخدم غير موجود" },
          { status: 401 }
        );
  
      if (user.verified)
        return NextResponse.json(
          { error: "خطأ في الطلب ,تم التأكيد مسبقا" },
          { status: 401 }
        );
      ...
      return NextResponse.json({ message: "يرجى التحقق من البريد الإلكتروني" });
    } catch (error) {
      return NextResponse.json(
        {
          error: "لايمكن تأكيد البريد الإلكتروني ، خطأ في العملية!",
